...
class DashboardController extends Controller {
    ...
    public function index() {
        ...
        if ($user_type == 'customer') {
            ...
            FROM currency LEFT JOIN transactions ON currency.id=transactions.currency_id WHERE currency.status=1 GROUP BY currency.id,currency.name,currency.exchange_rate");
        } else {
